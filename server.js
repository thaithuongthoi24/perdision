...
app.post('/predict', async (req, res) => {
    const { area,num_bed,num_bath,school,hospital,marketplace,pharmacy,supermarket,university,lat,lon } = req.body;
    const val  = await geoCoding(lat,lon);
    console.log(val.region)
    const point = placePoint(val.region, val.county);   
    const childProcess = spawn('python', ['./py-code/price-predict.py', area,num_bed,num_bath,school,hospital,marketplace,pharmacy,supermarket,university,point])
        var predictedPrice;
...
geoCoding = (lat, lon) => {
    return new Promise((resolve, reject) => {
        request(`https://apis.wemap.asia/we-tools/pip/${lon}/${lat}?key=vpstPRxkBBTLaZkOaCfAHlqXtCR`, (error, header, body) => {
            if (error) {
                reject(error)
            }
            else {
                try {
                    ...
                    console.log({ region: result.region[0].name, county: result.county[0].name });
                    resolve({ region: result.region[0].name, county: result.county[0].name })
                } catch (e) {
                    reject(e)
                }
            }
...
placePoint = (region, county) => {
    var pointF;
    dataKV.map(e => {
        if (e.province === region.trim()) {
            pointF = e.point
            console.log(pointF)
            e.cl.map(t => {
                t.name.map(v => {
                    if (v === county.trim()) {
                        pointF = pointF * (t.code)
                    }
                })
            })
        }
